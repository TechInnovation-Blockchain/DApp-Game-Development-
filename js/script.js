...
var jsCopter = {
    ...
    options : {
        ...
        copter : {
            ...
            acceleration : 0.1,            // how much to increase the speed by each time the game refreshes and the button is held down
            img : "images/plane.png"                      // optional copter image path, relative to the html page
        },
        physics : {
    ...
    createCopter: function() {
        ...
        // mouse button not held down
            ...
            this.gameData.copter.speed = 0;
            this.gameData.copter.y = this.gameData.copter.y;
        }
        // set new Y position
    ...
    createSplash: function(){
        var draw = this.drawContext;
        var copter = new Image();
        copter.src = "./images/splash.png";

        copter.onload = function() {
            draw.drawImage(copter, 600, 400, 461, 93);
        }
    },
    /*
    ...
    checkForImpact: function() {
        ...
        if (this.gameData.obstacles.current.length >=1) {
            ...
            for (var x = this.gameData.obstacles.current.length-1; x >= 0; x--){
                ...
                if(
                    ...
                    this.gameData.obstacles.current[x].x <= parseInt(this.gameData.copter.x + 100)
                ) {
                    // condition : check for impacts on obstacles in range
                    if (
                        parseInt(this.gameData.copter.y + 38) >= this.gameData.obstacles.current[x].y &&
                        this.gameData.copter.y <= parseInt(this.gameData.obstacles.current[x].y + this.options.obstacles.height)
                    ) {
                        return true;
                    }
                }
        ...
        for (var i = 0; i < this.gameData.walls.current.length; i++){
            ...
            if (this.gameData.walls.current[i].x < this.options.walls.width + this.options.copter.width) {
                // console.log(this.options.walls.width);
    ...
    endGame: function() {
        ...
        if (!this.deathText) {
            ...
            var deathTextText = document.createTextNode("Crashed!");
